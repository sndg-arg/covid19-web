...
for item in pbar:
    ...
    if primers:
        ...
        cmd = f'python3 /app/script/MSAMap.py -r {ref_id}  -i {msa} > "{out}/samples/{sample}/{sample}_variants.vcf" 2>>"{out}/samples/{sample}/log.txt"'
        exec(cmd, verbose=verbose)
        ...
        cmd = f'java -jar /app/snpEff/snpEff.jar -stats {out}/samples/{sample}/snpEff.html rsv  "{out}/samples/{sample}/{sample}_ann.tmp" > "{out}/samples/{sample}/{sample}_ann.vcf" 2>>"{out}/samples/{sample}/log.txt"'
        exec(cmd, verbose=verbose)
