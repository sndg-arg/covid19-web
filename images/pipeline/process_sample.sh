...
echo fastp -f $TRIML -t $TRIMR -l $MINLEN -e $QPROM -n $NPROM --thread $CPUS -i $IN1 -I $IN2 -o $OUT1 -O $OUT2 >> ${RESULTS}/${SAMPLE_NAME}/log.txt
...
dedupe.sh threads=$CPUS overwrite=t in=$IN1 out=$OUT1 > $LOG1  2>&1
dedupe.sh threads=$CPUS overwrite=t in=$IN2 out=$OUT2 > $LOG2  2>&1
rm $IN1 $IN2
...
OUT3=${RESULTS}/${SAMPLE_NAME}/call_consensus.vcf.gz
...
bcftools view  --output-type b -i  'DP>10' $OUT2 > $OUT3 2>>${RESULTS}/${SAMPLE_NAME}/log.txt
rm $OUT1
...
bcftools consensus -f $REFERENCE -H A -o ${OUT1}.tmp $IN1 2>>${RESULTS}/${SAMPLE_NAME}/log.txt
result=$(python3 <<EOF
import Bio.SeqIO as bpio
r = bpio.read('${OUT1}.tmp','fasta')
r.description = ''
r.name = ''
...
#Denovo
echo "denovo" >> ${RESULTS}/${SAMPLE_NAME}/log.txt
IN1=${RESULTS}/${SAMPLE_NAME}/1_repaired.fastq
IN2=${RESULTS}/${SAMPLE_NAME}/2_repaired.fastq
OUT2=${RESULTS}/${SAMPLE_NAME}/denovo
spades.py --meta -t $CPUS --pe-1 1 $IN1 --pe-2 1 $IN2 -o $OUT2 >> ${RESULTS}/${SAMPLE_NAME}/log.txt   2>&1
python3 -c "import Bio.SeqIO as bpio;bpio.write([x for x in bpio.parse('${OUT2}/scaffolds.fasta','fasta') if len(x) > 300  ],'${OUT2}.fna','fasta') "
rm -r $OUT2 $IN1 $IN2

#Assembly vs ref
IN=${RESULTS}/${SAMPLE_NAME}/denovo.fna
TMP1=${RESULTS}/${SAMPLE_NAME}/nucdifftmp
nucdiff --vcf yes  /ref/genome.fna $IN $TMP1 $SAMPLE_NAME >> ${RESULTS}/${SAMPLE_NAME}/log.txt   2>&1
